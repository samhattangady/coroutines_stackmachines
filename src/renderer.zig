...
pub const Renderer = struct {
    ...
    pub fn render_app(self: *Self, ticks: u32, app: *App) void {
        ...
        for (app.groups.items) |group| self.draw_group(group, self.camera);
        self.draw_buffers();
        if (!WEB_BUILD) {
    ...
    fn draw_shape(self: *Self, shape: Shape, position: Vector2, camera: *const Camera) void {
        const base: c_uint = @intCast(c_uint, self.base_shader.triangle_verts.items.len);
        const render_size = camera.render_size();
        for (shape.vertices.items) |vertex| {
            const vpos = vertex.position.added(shape.position).added(position);
            const pos = self.screen_pixel_to_gl(vpos, render_size);
            self.base_shader.triangle_verts.append(.{ .position = pos, .color = shape.color }) catch unreachable;
        }
        for (shape.indices.items) |index| self.base_shader.indices.append(base + index) catch unreachable;
        if (shape.is_active) {
            const active = Vector4_gl{ .x = 1.0, .y = 0.2, .z = 0.2, .w = 1.0 };
            const vertex = Vector4_gl{ .x = 1.0, .y = 1.0, .z = 0.2, .w = 1.0 };
            for (shape.vertices.items) |vtx, i| {
                const j = if (i == shape.vertices.items.len - 1) 0 else i + 1;
                const p0 = vtx.position.added(shape.position).added(position);
                const p1 = shape.vertices.items[j].position.added(shape.position).added(position);
                self.draw_line(p0, p1, active, 2, camera);
                self.draw_circle(p0, 5, vertex, camera);
            }
        }
    }

    fn draw_group(self: *Self, group: ShapeGroup, camera: *const Camera) void {
        for (group.shapes.items) |shape| {
            self.draw_shape(shape, group.position, camera);
        }
        if (group.is_active) {
            const p0 = group.position.added(Vector2{ .x = group.bounding_box[0].x, .y = group.bounding_box[0].y });
            const p1 = group.position.added(Vector2{ .x = group.bounding_box[0].x, .y = group.bounding_box[1].y });
            const p2 = group.position.added(Vector2{ .x = group.bounding_box[1].x, .y = group.bounding_box[1].y });
            const p3 = group.position.added(Vector2{ .x = group.bounding_box[1].x, .y = group.bounding_box[0].y });
            const active = Vector4_gl{ .x = 1, .y = 1, .z = 1, .w = 1.0 };
            self.draw_line(p0, p1, active, 2, camera);
            self.draw_line(p1, p2, active, 2, camera);
            self.draw_line(p2, p3, active, 2, camera);
            self.draw_line(p3, p0, active, 2, camera);
        }
    }

    fn draw_circle(self: *Self, pos: Vector2, radius: f32, color: Vector4_gl, camera: *const Camera) void {
        const half_size = Vector2{ .x = radius, .y = radius };
        const p1 = self.screen_pixel_to_gl(Vector2.subtract(pos, half_size), camera.render_size());
        const p2 = self.screen_pixel_to_gl(Vector2.add(pos, half_size), camera.render_size());
        const t1: f32 = 1.0 / @intToFloat(f32, CIRCLE_TEXTURE_SIZE);
        const t2: f32 = 1.0;
        const base = @intCast(c_uint, self.base_shader.triangle_verts.items.len);
        self.base_shader.triangle_verts.append(.{ .position = p1, .color = color, .texCoord = .{ .x = t1, .y = t1 } }) catch unreachable;
        self.base_shader.triangle_verts.append(.{ .position = .{ .x = p2.x, .y = p1.y, .z = p1.z }, .color = color, .texCoord = .{ .x = t2, .y = t1 } }) catch unreachable;
        self.base_shader.triangle_verts.append(.{ .position = p2, .color = color, .texCoord = .{ .x = t2, .y = t2 } }) catch unreachable;
        self.base_shader.triangle_verts.append(.{ .position = .{ .x = p1.x, .y = p2.y, .z = p1.z }, .color = color, .texCoord = .{ .x = t1, .y = t2 } }) catch unreachable;
        const indices = [_]c_uint{ base + 0, base + 1, base + 3, base + 1, base + 2, base + 3 };
        self.base_shader.indices.appendSlice(indices[0..6]) catch unreachable;
    }
